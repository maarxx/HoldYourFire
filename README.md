...
Unzip it, and it will spew out a single folder, which is probably named something like `HoldYourFire-master`.
...
`C:\Program Files (x86)\Steam\steamapps\common\RimWorld\Mods\HoldYourFire-master\Assemblies`
...
`C:\Program Files (x86)\Steam\steamapps\common\RimWorld\Mods\HoldYourFire-master`
...
To do this, just click the button where it says "Branch: master", and then click the option for "beta". Congratulations! You've changed branches! Follow the same steps to download and install, except instead of `HoldYourFire-master` it will now be `HoldYourFire-beta`. You can have both versions installed, but please don't try to have both versions enabled at once using the in-game Mod menu.
